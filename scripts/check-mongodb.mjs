...
try {
  ...
  console.log(JSON.stringify({ connected: true, liveProducts: await db.collection("products").countDocuments({ catalog: { $ne: null }, syncStatus: { $ne: "removed" } }), deletionPending: await db.collection("products").countDocuments({ syncStatus: "deletion-pending" }) }));
