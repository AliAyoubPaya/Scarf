...
WooCommerce products and variations → authenticated sync / signed webhooks → MongoDB `products` snapshots → Next.js homepage, collections and product pages.
...
- Vercel's existing `scarf` project has a connected MongoDB Atlas resource named `scarf-catalog`, shown as **Free Plan**. `MONGODB_URI` is configured for all environments. Database connectivity and actual catalog writes have **not yet been verified**.
...
3. Keep database credentials in Vercel's encrypted environment settings. Set `MONGODB_URI`, `MONGODB_DATABASE=scarf`, and initially `CATALOG_MODE=mongodb-preview`. Verify the integration's actual generated variable name and map it to `MONGODB_URI` if needed. Do not put the URI into a `NEXT_PUBLIC_*` variable.
...
5. Put the same preview credentials in ignored `.env.local` locally (never paste them in chat). Node 22+ commands:

```text
node --env-file=.env.local scripts/check-mongodb.mjs
node --env-file=.env.local --experimental-strip-types scripts/seed-preview-catalog.mjs --confirm
```

The seed script only upserts the 10 fixture products in **`demo_products`**, with `wooId: null`, `synced: false`, and `purchasable: false`. It never writes live `products`. Existing documents with the same demo IDs are replaced; unrelated records are untouched. The preview is suitable for demonstrating colour switching while WooCommerce is unavailable.
...
| CATALOG_MODE | Read source | Purchase actions |
| --- | --- | --- |
| `demo` (default) | Local sample catalog | Disabled |
| `mongodb-preview` | MongoDB `demo_products` | Disabled |
| `mongodb` | MongoDB `products`, populated by sync | Enabled only with the commerce configuration and synced purchasable options |
...
3. Upload `wordpress/hs-headless-checkout/` as a WordPress plugin, activate it, and define `HS_CHECKOUT_SECRET` in the site's private `wp-config.php`. It must match `WOO_CHECKOUT_SECRET` in Next.js. Do not commit the value. The plugin uses an HMAC-authenticated endpoint, signed request age checks and an expiring single-use transfer handle. No payment data or API credentials are put into the URL.
4. Run a protected initial sync: `POST /api/catalog/sync`, `Authorization: Bearer <CATALOG_SYNC_SECRET>`, JSON `{ "page": 1 }`. Follow each returned `nextPage` until null. Batches are small to fit serverless limits; individual products with many variations may require a longer worker execution window. For targeted repair use `{ "productId": 123 }`. Repeating a product sync replaces the snapshot safely.
5. Add WooCommerce webhooks for product created/updated/deleted (and restored if configured), pointing to `https://YOUR-STOREFRONT/api/woocommerce/webhook`, using `WOOCOMMERCE_WEBHOOK_SECRET`. Invalid signatures are rejected. Canonical parent data is re-fetched before storing, preventing stale payload replay. Configure parent update delivery for variation-only and stock changes on the eventual WooCommerce setup; verify these events in staging. Periodic reconciliation for missed deliveries should be scheduled separately after approval. Existing snapshots whose products are deleted are tombstoned on their deletion webhook/targeted sync; bulk pagination alone is not a deletion sweep.
...
7. Switch `CATALOG_MODE=mongodb` after the real catalog is imported. Set `WOO_CHECKOUT_ENABLED=true` only after the staging checklist below passes.
...
- Atlas connectivity, seeded preview read, no fixture fallback on DB outage.
