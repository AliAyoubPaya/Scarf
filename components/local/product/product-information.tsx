  ...
  async function share () {
    try {
      ...
      if (!(error instanceof DOMException && error.name === 'AbortError'))
        setNotice(
          'Copy the page address from your browser to share this scarf.'
        )
    }
  }
  ...
  return (
    <div className='min-w-0 md:pt-4 lg:sticky lg:top-36 lg:self-start [&_button]:disabled:cursor-not-allowed [&_button]:disabled:opacity-45'>
      <div className='flex items-center justify-between gap-3'>
        ...
        <button
          type='button'
          aria-label='Share this scarf'
          onClick={share}
          className='flex size-11 shrink-0 items-center justify-center rounded-full border border-brand-gold-line hover:bg-brand-gold-soft'
        >
          <Share2 className='size-4' strokeWidth={1.5} />
        </button>
      </div>
      <h1 className='mt-3 max-w-lg font-heading text-[2.15rem] font-light leading-[1.12] tracking-[-0.035em] sm:text-4xl lg:text-[2.8rem]'>
        {product.name}
      </h1>
      <div
      ...
      {(product.variants?.length || 0) > 1 ? (
        <nav aria-label='Product options' className='mb-6'>
          <p className='text-xs text-[#71695d]'>
            Size / option{' '}
            <span className='ml-2 text-[#302a23]'>{variant.label}</span>
          </p>
          <div className='mt-3 flex flex-wrap gap-2 [&_a]:flex [&_a]:min-h-11 [&_a]:min-w-11 [&_a]:items-center [&_a]:justify-center [&_a]:rounded [&_a]:border [&_a]:px-3 [&_a]:text-xs'>
            {product.variants?.map(option => (
              <Link
                key={option.id}
                href={productOptionHref(product, option)}
                scroll={false}
                ...
                className={
                  option.id === variant.id
                    ...
                    : 'border-[#ded6c8] hover:border-brand-gold-ink'
                }
              >
                {option.label}
                {option.stockStatus === 'sold-out' ? ' · Sold out' : ''}
              </Link>
            ))}
          </div>
        </nav>
      ...
      <button
        ...
        onClick={() => purchase()}
        className='mt-4 flex min-h-14 w-full items-center justify-center gap-3 rounded bg-brand-gold-ink px-4 font-heading text-sm tracking-wide text-white hover:bg-[#604c22]'
      >
      ...
      <button
        ...
        className='mt-3 flex min-h-12 w-full items-center justify-center gap-3 rounded border border-brand-gold-line px-4 text-sm hover:bg-brand-gold-soft'
      >
        {pending === 'checkout' ? 'Opening checkout…' : 'Buy now'}
        <ArrowRight className='size-4' />
      </button>
      <p className='mt-3 text-center text-xs leading-5 text-[#82796a]'>
        {ready && product.commerce?.synced
          ? 'Secure checkout on our WooCommerce store. Delivery and taxes are confirmed at checkout.'
          : 'Online shopping is being connected. Explore colours now; checkout will be available once the store is ready.'}
      </p>
      <div
        aria-live='polite'
        className='mt-3 text-sm leading-6 text-brand-gold-ink'
      >
        {notice}
        {added ? (
      ...
      <div className='mt-5 flex items-center gap-3 rounded-lg bg-[#f3efe6] p-4 text-xs leading-5'>
        <LockKeyhole
          className='size-5 shrink-0 text-brand-gold-ink'
          strokeWidth={1.3}
        />
        <p>
          Need a little help with your choice?
          <br />
          <Link
            href='/pages/contact'
            className='text-brand-gold-ink underline underline-offset-4'
          >
            Talk to the HS by Saman team
          </Link>
        </p>
      </div>
      <div className='mt-6 divide-y divide-[#e6e0d6] border-y border-[#e6e0d6] [&_summary]:flex [&_summary]:min-h-14 [&_summary]:cursor-pointer [&_summary]:list-none [&_summary]:items-center [&_summary]:justify-between [&_summary]:gap-3 [&_summary]:font-heading [&_summary]:text-sm [&_summary::-webkit-details-marker]:hidden [&_summary_svg]:size-4 [&_details[open]_summary_svg]:rotate-180 [&_details>div]:pb-5 [&_details>div]:text-sm [&_details>div]:leading-6 [&_details>div]:text-[#71695d]'>
        <details open>
          ...
          <div>
            <dl className='grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 [&_dd]:text-right'>
              <dt>Fabric collection</dt>
              <dd>{product.fabric}</dd>
              <dt>Shade / option</dt>
              <dd>{variant.label}</dd>
              <dt>Product reference</dt>
              <dd>HS-{product.source.externalId}</dd>
            </dl>
            <p className='mt-4 text-xs leading-5'>
              Please confirm exact composition and dimensions with our team.
              Colours can appear different in photographs and on different
              screens.
            </p>
          </div>
        </details>
        <details>
          ...
          <div>
            Always follow the care label supplied with your scarf. Ask our team
            for fabric-specific washing and ironing instructions before
            cleaning.
          </div>
        </details>
        <details>
          ...
          <div>
            Shipping options and charges are calculated in WooCommerce checkout.
            For exchange eligibility and delivery estimates,{' '}
            <Link
              href='/pages/contact'
              className='text-brand-gold-ink underline underline-offset-4'
            >
      ...
      <div className='fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-3 border-t border-brand-gold-line bg-[#fbfaf8]/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md md:hidden'>
        <div className='min-w-0'>
          <p className='truncate text-xs text-[#71695d]'>{variant.label}</p>
          <p className='mt-1 font-heading text-sm'>{price}</p>
        </div>
        <button
          type='button'
          disabled={!canBuy || !!pending}
          onClick={() => purchase()}
          className='min-h-11 shrink-0 rounded bg-brand-gold-ink px-5 font-heading text-xs text-white'
        >
          {pending ? 'Please wait…' : soldOut ? 'Sold out' : 'Add to bag'}
        </button>
      </div>
    </div>
  )
